...
export default function ProjectCard({ project }: ProjectCardProps) {
  ...
  return (
    <motion.a
      ...
      whileTap={{ scale: 0.98 }}
      className="relative dashed-card rounded-none p-4 flex flex-col justify-between group cursor-pointer transition-all duration-300 hover:border-dashed hover:border-white/60 hover:shadow-[0_0_20px_rgba(255,255,255,0.06)]"
    >
      ...
      <div>
        {/* Top Image Preview Banner (Sharp Edges) */}
        ...
        <div className="flex items-center justify-between gap-2 mb-2">
          <span className="text-[11px] font-mono text-[#8a8a93] truncate">
            {project.category}
          </span>
          <span
        ...
        <h3 className="font-sans font-bold text-base sm:text-lg text-white mb-2 tracking-tight group-hover:text-white flex items-center justify-between">
          ...
          <ExternalLink className="w-4 h-4 text-[#8a8a93] opacity-0 group-hover:opacity-100 group-hover:text-white transition-all duration-200" />
        </h3>

        {/* Description */}
        <p className="text-xs text-[#909095] font-mono leading-relaxed mb-4 min-h-[56px] opacity-90">
          {project.description}
        </p>
      </div>
